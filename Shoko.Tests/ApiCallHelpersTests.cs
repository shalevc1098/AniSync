...
public class ApiCallHelpersTests
{
    ...
    // ========================================================================
    // Response disposal on non-success
    // Non-success responses should return null (existing behavior preserved).
    // ========================================================================

    [Fact]
    public void GetProviderIds_Non_Success_Returns_Null()
    {
        var response = new HttpResponseMessage(HttpStatusCode.NotFound);
        bool isSuccess = response.IsSuccessStatusCode;
        response.Dispose();

        isSuccess.Should().BeFalse("404 is not a success status code");
    }

    [Fact]
    public void GetProviderIds_Success_Should_Not_Return_Null()
    {
        var response = new HttpResponseMessage(HttpStatusCode.OK);
        bool isSuccess = response.IsSuccessStatusCode;
        response.Dispose();

        isSuccess.Should().BeTrue("200 is a success status code");
    }

    // ========================================================================
    // Retry loop disposal
    // Previous responses must be disposed before retrying.
    // ========================================================================

    [Fact]
    public void RetryLoop_Disposes_Previous_Response()
    {
        ...
        var disposeAgain = () => response.Dispose();
        disposeAgain.Should().NotThrow("disposing an already-disposed response should be safe");
    }

    [Fact]
    public void RetryLoop_Null_Response_Should_Increment_Attempts()
    {
        ...
        attempts.Should().Be(1, "null response should increment attempts");
    }

    // ========================================================================
    // HttpResponseMessage disposal
    // Verify that using declarations properly dispose HttpResponseMessage.
    // ========================================================================

    [Fact]
    public void HttpResponseMessage_Using_Disposes_Correctly()
    {
        var response = new HttpResponseMessage(HttpStatusCode.OK);
        response.Dispose();

        var disposeAgain = () => response.Dispose();
        disposeAgain.Should().NotThrow("disposing an already-disposed HttpResponseMessage should be safe");
    }
}